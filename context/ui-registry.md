...
- **Dipakai di:** ProgramStack (route "GotvForm", push dari tombol "+ Input Kegiatan Baru")
...
- **Catatan:** Item #3 "saran konsultan politik" (`build-plan.md` § Fase Berikutnya), dikerjakan 2026-08-22. **2 keputusan skema final** (lihat `progress-tracker.md` Decisions untuk kronologi lengkap — sempat berubah beberapa kali sebelum landing di sini):
  ...
  - **Ringkasan/realisasi-pos/tren adalah mock STATIS per scope** (`services/budgeting.ts`), SENGAJA tidak diturunkan dari daftar transaksi (canvas sumbernya sendiri juga tidak menghubungkan keduanya — `txCount` 84 tidak match 4 transaksi contoh). Cuma daftar "Pengeluaran Terbaru" yang benar-benar mutable (transaksi baru masuk paling atas via `createBudgetTransaction`).
  - **Transaksi baru selalu berstatus "Menunggu"** (judgment call, didokumentasikan — tidak ada UI approve/tolak di canvas maupun scope sesi ini, alur approval sungguhan masih di luar scope).
  ...
  - Pola mock service STANDAR (`services/budgeting.ts`, array in-memory + `isMockApiEnabled()`) — sama persis `rivalcaleg.ts`/`saksi.ts`/`realcount.ts`, tidak ada backend/repo untuk modul ini (CLAUDE.md Aturan #6).
