...
- **Pola kelas kunci:** `ScrollView` + 3 tab (Kekuatan/Daftar Rival/Aktivitas) — struktur SAMA PERSIS `IsuAspirasiScreen` (tab pill `bg-surface-secondary`→`bg-primary` aktif, `RefreshControl`, `headerRight` icon download `Alert` "Segera hadir" pola `BudgetingKampanyeScreen"`). Tab "Daftar Rival" dapat search bar client-side (filter nama/partai). `useHideTabBar()` dipasang (root screen, pola sama Tokoh/Budgeting/Quick Count/dst.).
